...
- filter:clear event on div[filter-name], which is called by the "Remove all filters" button, clears this filter;
...
@push('crud_list_scripts')
	<script>
		jQuery(document).ready(function($) {
			$("div[filter-key={{ $filter->key }}] .from, div[filter-key={{ $filter->key }}] .to").change(function(e) {
				e.preventDefault();
				var from = $("div[filter-key={{ $filter->key }}] .from").val();
				var to = $("div[filter-key={{ $filter->key }}] .to").val();
				if (from || to) {
					var range = {
						'from': from,
						'to': to
			...
			$('div[filter-key={{ $filter->key }}]').on('filter:clear', function(e) {
				...
				$("div[filter-key={{ $filter->key }}] .from").val("");
				$("div[filter-key={{ $filter->key }}] .to").val("");
				$("div[filter-key={{ $filter->key }}] .to").trigger('change');
			});
