            ...
            $('#filter_{{ $filter->key }}').each(function () {
            	...
            	$(this).select2({
					...
					if (URI(new_url).hasQuery(filterName, true)) {
						$('div[filter-key='+filterKey+']').removeClass('active').addClass('active');
					}
					else
					{
						$("div[filter-key="+filterKey+"]").removeClass("active");
						$("div[filter-key="+filterKey+"]").find('.dropdown-menu').removeClass("show");
					}
				});

				// when the dropdown is opened, autofocus on the select2
				$('div[filter-key='+filterKey+']').on('shown.bs.dropdown', function () {
					$('#filter_'+filterKey).select2('open');
				});

				// clear filter event (used here and by the Remove all filters button)
				$('div[filter-key='+filterKey+']').on('filter:clear', function(e) {
					$('div[filter-key='+filterKey+']').removeClass('active');
