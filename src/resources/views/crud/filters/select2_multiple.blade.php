...
<div filter-name="{{ $filter->name }}"
    ...
    <div class="dropdown-menu p-0">
      <div class="form-group backpack-filter mb-0">
			<select id="filter_{{ $filter->key }}" name="filter_{{ $filter->key }}" data-filter-key="{{ $filter->key }}" data-filter-name="{{ $filter->name }}" class="form-select input-sm
    </button> select2" data-filter-type="select2_multiple" placeholder="{{ $filter->placeholder }}" multiple>
            ...
            $('select[name=filter_{{ $filter->key }}]').not('[data-filter-enabled]').each(function () {
                ...
                $(this).change(function() {
					...
					if (URI(new_url).hasQuery(filterName, true)) {
						$("div[filter-key="+filter_key+"]").removeClass('active').addClass('active');
					}
					else
					{
						$("div[filter-key="+filter_key+"]").removeClass("active");
						$("div[filter-key="+filter_key+"]").find('.dropdown-menu').removeClass("show");
					}
				});

				// when the dropdown is opened, autofocus on the select2
				$("div[filter-key="+filter_key+"]").on('shown.bs.dropdown', function () {
					$('#filter_'+filter_key+'').select2('open');
				});

				// clear filter event (used here and by the Remove all filters button)
				$("div[filter-key="+filter_key+"]").on('filter:clear', function(e) {
