        ...
        <div class="d-flex gap-2 flex-wrap">
          <!-- THE ACTUAL FILTERS -->
    			@foreach ($crud->filters() as $filter)
    				@include(stoke_view($filter->getViewWithNamespace()))
    			@endforeach
            <div class="btn-group">
                <p class="nav-item" style="padding-top: 3px; "><a href="#" id="remove_filters_button" class="nav-link {{ count(Request::input()) != 0 ? '' : 'invisible' }}"><i class="la la-eraser"></i> {{ trans('backpack::crud.remove_filters') }}</a></p>
            </div>
        </div>
      </div><!-- /.navbar-collapse -->
  </nav>
...
@push('crud_list_scripts')
    ...
    <script>
      ...
      jQuery(document).ready(function($) {
      	$("#remove_filters_button").click(function(e) {
      		e.preventDefault();
		    	...
		    	var ajax_table = $("#crudTable").DataTable();
  				...
  				// clear all filters
  				$(".navbar-filters div[filter-name]").trigger('filter:clear');
        ...
        $(".navbar-filters div[filter-name]").on('filter:clear', function() {
          var anyActiveFilters = false;
          $(".navbar-filters div[filter-name]").each(function () {
